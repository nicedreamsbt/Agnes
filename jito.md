...
../_images/matrixtxnsv4.png
System Overview
../_images/systemdiagramv2.png
...
The auction creates a stream of bundles that maximizes tips in a block. Parallelism in locking patterns is leveraged where possible to allow for local state auctions.
...
Please note that Jito enforces a minimum tip of 1000 lamports for bundles. During high-demand periods, this minimum tip might not be sufficient to successfully navigate the auction, so it’s crucial to set both a priority fee and an additional Jito tip to optimize your transaction’s chances. Please see Tip Amounts
...
REQUIRED: First Transaction Signature embedded in the transaction, as base64 (recommended) or base58 (slow, DEPRECATED) encoded string.

encoding

string

OPTIONAL: Encoding used for the transaction data. Values: base64 (recommended) or base58 (slow, DEPRECATED). Default: base58
...
The result will be the same as described in the Solana RPC documentation. If sending as a bundle was successful, you can get the bundle_id from the x-bundle-id HTTP header in the response.
...
REQUIRED: Fully-signed transactions, as base64 (recommended) or base58 (slow, DEPRECATED) encoded strings. Maximum of 5 transactions.
...
REQUIRED: An array of bundle IDs to confirm. Maximum of 5.
...
Tip accounts as a list of strings.
...
Example
Check out this example bundle using the protection: jitodontfront bundle

Disclaimer: This feature may help reduce sandwich attacks but is not guaranteed to do so and is not a solution for all variations in transaction ordering, including any ordering conducted by third parties. In addition, this feature applies only to the block engine and not to any other software.

Tips
Best Practices for Tipping:

Integrate tip instructions within your main transaction alongside core logic.

If transaction space is constrained, implement post-transaction checks or add assertions in a separate tipping transaction to verify expected states.

Exercise caution when sending tips as standalone transactions in bundles, as this may increase vulnerability to uncle bandit scenarios.
...
Tip Dashboard
To view dashboard please click here .
...
Custom Rate Limit Request
Please open a ticket to request a new rate limit or a rate limit increase Open a Ticket

Troubleshooting
Priority Fee
Q: How to know how much to set priority fee?

Please see Tip Amount for sendTransaction
...
If you have the bundleID, you can look over Jito explorer, otherwise you can see basic inflight knowledge within the last 5 minutes through getInflightBundleStatuses.

The minimum tip is 1000 lamports, but if you’re targeting a highly competitive MEV opportunity, you’ll need to be strategic with your tip. Consider adjusting your tip amount, based on the current pricing of tips in the system, and also take into account the latency to each component you’re interacting with to maximize your chances of success.
...
You protect yourself from bundles being ‘unbundled’. This means having pre/post account checks and other safeguards that’ll protect you in the case your bundle is rebroadcasted.

Make sure you minimize your bundles from being leaked; if you send to Jito you are never going to be explicitly leaked by Jito. However, be aware that during uncled block situations you are exposed to this.

Always make sure your Jito tip transaction is in the same transaction that is running the MEV strategy; this way if the transaction fails, you don’t pay the Jito tip:
...
© Copyright 2024, Jito Labs. Last updated on Apr 07, 2026.
